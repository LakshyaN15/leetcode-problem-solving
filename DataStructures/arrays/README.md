...
| 2 | [Product Of Array Except Self](https://leetcode.com/problems/product-of-array-except-self/) | 🟡 Medium | Array, Prefix Sum | 1. Brute Force - declare 2 var i, j and itr them till end. If i==j , continue(skip) , product*=nums[j]. Add to res[i] or res.append. Return res in end<br>TC: O(n^2)<br>SC: O(1) extra space and O(n) for output array<br>2. Prefix and Suffix - declare prefix and suffix as 1 , declare res as [0] * n array, store prefix from right to left in res and suffix after from left to right<br>TC: O(n)<br>SC: O(1), excluding op array | [Python](238-product-of-array-except-self/product-of-array-except-self.py) |
| 3 | [Move Zeroes](https://leetcode.com/problems/move-zeroes/) | 🟢 Easy | Array, Two Pointers | 1. Extra Spacing -> declare a temp array and append if num !=0. Declare a for loop and if i<len(tmp) then nums[i]=tmp(i) else nums[i]=0. Example 1,2,3,0,0 and tmp=1,2,3 (first 3 pos are filled for last 2 0's it goes to else condition)<br>TC: O(n)<br>SC: O(n)<br>2. Two Pointer (Two Pass) -> declare l=0 , itr r in range len(nums). Push if nums[r] != 0 and increase l by 1. While loop to cmp l<len(nums) and put nums[l] as 0 and itr till condition false.<br>TC: O(n)<br>SC: O(1)<br>3. Two Pointers (One Pass) -> Do a swap nums[l], nums[r]=nums[r], nums[l]<br>TC: O(n)<br>SC: O(1) | [Python](283-move-zeroes/move-zeroes.py) |
| 4 | [Maximum Average Subarray I](https://leetcode.com/problems/maximum-average-subarray-i/) | 🟢 Easy | Mid Level, Array, Sliding Window | 1. Sum first K elements, slide across array -> add new element Subtract the element leaving window, track max sum seen and divide final max by k to get avg<br>TC: O(n) - Scan array once<br>SC: O(1) - No extra space | [Python](643-maximum-average-subarray-i/maximum-average-subarray-i.py) |
<!-- PROBLEMS:END -->
